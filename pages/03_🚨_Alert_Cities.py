"""
Merged Page: Risk & Alert Cities.
Combines the former Alert Cities and Risk Alerts pages into one page.
"""
...
st.set_page_config(
    page_title="Risk & Alert Cities | Air Quality Predictor",
    page_icon="🚨",
    layout="wide",
)
prepare_page()

st.markdown("## 🚨 Risk & Alert Cities")
st.caption(
    "Unified national hotspot monitoring and real-time environmental risk alerts. "
    "This page combines city-level AQI risk diagnostics with the early-warning engine."
)

cities = load_indian_cities()
city_names = [c["city"] for c in cities]

# One city selector shared by both sections.
with st.sidebar:
...
alerts_tab, warning_tab = st.tabs(["🚨 Alert Cities & Hotspots", "⚠️ Risk Alerts & Early Warning"])

with alerts_tab:
    status_summary = get_india_wide_monitoring_status()
    df_india = status_summary["dataset_df"]
    df_alert = (
        df_india[df_india["aqi"] > 200]
    ...
    st.caption(
        "Automated registry of Indian cities exceeding safety thresholds "
    ...
    with c2:
        render_metric_card(
            ...
            f"{len(df_alert)}",
            f"{(len(df_alert) / status_summary['total_monitored']) * 100:.1f}% of national network",
            "Priority Action",
            "#ef4444",
        )
    with c3:
        render_metric_card(
            "Severe Category (AQI > 400)",
            f"{severe_count}",
    ...
    with c4:
        ...
        render_metric_card(
            "Highest AQI Detected",
            f"{status_summary['highest_aqi_val']}",
            f"{highest_city} ({status_summary['highest_aqi_category']})",
            "Maximum Spike",
            "#f97316",
        )
    ...
    if filter_sev == "Severe Only (AQI > 400)":
        ...
        df_table_filtered = df_table_filtered[
            (df_table_filtered["aqi"] > 300) & (df_table_filtered["aqi"] <= 400)
        ]
    elif filter_sev == "Poor (201 - 300)":
        df_table_filtered = df_table_filtered[
            (df_table_filtered["aqi"] > 200) & (df_table_filtered["aqi"] <= 300)
        ]

    cols_to_show = [
        "city", "state", "aqi", "aqi_category", "dominant_pollutant",
        "pm25", "pm10", "temperature", "humidity", "risk_level", "timestamp"
    ]
    rename_map = {
        "city": "City", "state": "State", "aqi": "NAQI",
        "aqi_category": "Category", "dominant_pollutant": "Dominant Pollutant",
        "pm25": "PM2.5 (µg/m³)", "pm10": "PM10 (µg/m³)",
        "temperature": "Temp (°C)", "humidity": "Hum (%)",
        "risk_level": "Risk Level", "timestamp": "Last Synced",
    }
    st.dataframe(
    ...
    if not df_alert.empty:
        ...
        selected_drilldown = st.selectbox(
            "Select a City to Diagnose Causality & Source Breakdown",
            alert_city_list,
            key="alert_drilldown_city",
        )
        ...
        with diag_col1:
            ...
            <div class="telemetry-card" style="border-left: 4px solid #ef4444;">
                ...
                <div style="font-size: 13px; color: #f87171; margin: 4px 0;">
                    Calculated NAQI: <strong>{city_record['aqi']}</strong>
                    ({city_record['aqi_category'].upper()}) • Primary Driver:
                    <strong>{city_record['dominant_pollutant']}</strong>
                </div>
                ...
                <div style="font-size: 13px; color: #e2e8f0; line-height: 1.6;">
                    ...
                    • <em>Atmospheric Boundary Trapping:</em> High humidity
                    ({city_record['humidity']}%) and low surface wind restrict vertical
                    plume dispersion, trapping emissions near breathing zones.
                </div>
        ...
        with diag_col2:
            st.plotly_chart(
                render_driver_breakdown_chart(driver_res["drivers"]),
                use_container_width=True,
            )
    else:
...
with warning_tab:
    ...
    st.caption(
        "Automated threat evaluation engine screening live telemetry across "
        "7 discrete atmospheric risk triggers."
    )
    ...
    scenarios = [
        ("CASE 1: High-Risk AQI Threshold", "AQI >= 201 (Poor, Very Poor, Severe)",
         "Triggers immediate public alert and restriction of intense outdoor activities."),
        ("CASE 2: Rapid AQI Escalation", "Rate of change >= +30 AQI points in recent hours",
         "Warns of sudden atmospheric stagnation or severe localized industrial/biomass plume emission."),
        ("CASE 3: Hazardous Fine Particulate PM2.5", "PM2.5 >= 120 µg/m³ (2x National Standard)",
         "Issues HEPA air purifier guidance and mandatory N95 respirator advisory for sensitive groups."),
        ("CASE 4: Elevated Coarse Dust PM10", "PM10 >= 250 µg/m³",
         "Triggers municipal dust suppression (water misting on roads) and construction site shielding alerts."),
        ("CASE 5: Multi-Pollutant Co-Exposure", ">= 2 pollutants with Sub-Index >= 150 simultaneously",
         "Issues synergistic chemical hazard alert due to simultaneous particulate and gas phase toxicity."),
        ("CASE 6: Predicted Spike in Upcoming Horizon", "ML Forecast predicts future AQI >= 300 ahead",
         "Enables proactive commute and school schedule adjustments before peak pollution arrival."),
        ("CASE 7: Isolation Forest Atmospheric Anomaly", "Atypical multivariate outlier signature detected",
         "Flags unusual meteorological anomalies or unreported localized emission events."),
    ]
    for title, cond, impact in scenarios:
        ...
        <div class="telemetry-card" style="margin-bottom: 10px; padding: 12px 16px;">
            ...
            <div style="font-size: 12px; color: #cbd5e1; margin-top: 4px;">
                <strong>Trigger Condition:</strong> <code>{cond}</code>
            </div>
            <div style="font-size: 12px; color: #94a3b8; margin-top: 2px;">
                <strong>Operational Action:</strong> {impact}
            </div>
        </div>
        """, unsafe_allow_html=True)
